...
def main():
    ...
    client_private, client_public, prime, g = generate_dh_key()
    print(f"Client's public key: {client_public}")
    
    params = client_socket.recv(1024).decode().split(',')
    server_public = int(params[0])
    prime = int(params[1])
    g = int(params[2])
    print(f"Server's public key: {server_public}")
    
    client_socket.send(str(client_public).encode())
    

    shared_key = calculate_shared_secret(client_private, server_public, prime)
    ...
    decrypted_response, integrity_ok = decrypt_message(encrypted_response, shared_key)
