...
def f_function(block, subkey):
    ...
    for i in range(8):
        ...
        val = S_BOX[i][row][col]
        result += [int(x) for x in format(val, '04b')]
...
def bits_to_hex(bits):
    ...
    for i in range(0, len(bits), 8):
        byte = bits[i:i+8]
        hex_str += format(int(''.join(map(str, byte)), 2), '02x')
    return hex_str

def hex_to_bits(hex_str):
    bits = []
    for char in hex_str:
        bits += [int(x) for x in format(int(char, 16), '04b')]
    return bits
...
ciphertext_bits = des_encrypt(plaintext_bits, subkeys)
ciphertext_hex = bits_to_hex(ciphertext_bits)

decrypted_bits = des_decrypt(ciphertext_bits, subkeys)
